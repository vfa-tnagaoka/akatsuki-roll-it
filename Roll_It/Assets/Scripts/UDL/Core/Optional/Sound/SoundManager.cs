...
namespace UDL.Core.Sound
{
    ...
    public class SoundManager : MonoBehaviour
    {
        ...
        #region register local music

        //[SerializeField]
        //private AudioFile[] audioLocalFiles;

        private static SoundLocalAssets audioLocalAssets;
        ...
        public static int CurrentMusicID { get; private set; }
        ...
        public static bool MuteSound 
        {
            ...
            set
            {
                foreach (var audio in soundsAudio)
                {
                    audio.Value.Mute = value;
                }

                foreach (var audio in UISoundsAudio)
                {
                    audio.Value.Mute = value;
                }
        ...
        static SoundManager()
        {
            ...
            audioLocalAssets = Resources.Load<SoundLocalAssets>("Sounds/SoundLocalAssets");
            if (audioLocalAssets == null)
            {
                audioLocalAssets = Resources.Load<SoundLocalAssets>("Sound/SoundLocalAssets");
            }
            if (audioLocalAssets == null)
            {
                audioLocalAssets = Resources.Load<SoundLocalAssets>("SoundLocalAssets");
            }
            if (audioLocalAssets == null)
            {
                Debug.LogError("Can't find SoundLocalAssets");
            }
        }
        ...
        private void Init()
        {
            if (!initialized)
            {
                ...
                IgnoreDuplicateMusic = true;
                IgnoreDuplicateSounds = false;
                IgnoreDuplicateUISounds = false;

                initialized = true;

                if(audioLocalAssets == null) audioLocalAssets = Resources.Load<SoundLocalAssets>("SoundLocalAssets");
        ...
        private static Audio GetAudio(Audio.AudioType audioType, bool usePool, AudioClip audioClip)
        {
            ...
            foreach (int key in keys)
            {
                ...
                if(audio == null && usePool && audioPool.ContainsKey(key))
                {
                    audio = audioPool[key];
                }
        ...
        public static int PlayLocalMusic<T>(T audioKey, bool isFade = false) where T : System.Enum
        {
            return PlayLocalMusic(audioKey.ToString(), isFade);
        }

        public static int PlayLocalMusic(string audioKey, bool isFade = false)
        {
            AudioFile file = System.Array.Find(audioLocalAssets.MusicLocalFiles, AudioFile => AudioFile.audioName == audioKey);
            if (file == null || file.audioClip == null)
            {
                Debug.LogError("Local sound " + audioKey + " not found!");
                return -1;
            }
            var fadeValue = isFade ? 1f : 0f;
            return PlayMusic(file.audioClip, file.volume, file.isLooping, false, fadeValue, fadeValue);
        }

        public static int PlayLocalSound<T>(T audioKey) where T : System.Enum
        {
            
            return PlayLocalSound(audioKey.ToString());
        }

        public static int PlayLocalSound(string audioKey)
        {
            AudioFile file = System.Array.Find(audioLocalAssets.SoundLocalFiles, AudioFile => AudioFile.audioName == audioKey);
            if (file == null || file.audioClip == null)
            {
                Debug.LogError("Local sound " + audioKey + " not found!");
                return -1;
            }
            return PlaySound(file.audioClip, file.volume, file.isLooping, null);
        }
        ...
        private static int PlayAudio(Audio.AudioType audioType, AudioClip clip, float volume, bool loop, bool persist, float fadeInSeconds, float fadeOutSeconds, float currentMusicfadeOutSeconds, Transform sourceTransform)
        {
            ...
            if (audioType == Audio.AudioType.Music)
            {
                if(CurrentMusicID != audioID)
                {
                    ...
                    var audio = GetAudio(audioType, ignoreDuplicateAudio, audioID);
            ...
            {
                var audio = GetAudio(audioType, false, audioID);
